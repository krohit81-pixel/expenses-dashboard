...
| `CRON_SECRET`                            | Generate yourself: `openssl rand -hex 32`                                   | Added in v3.2.0, the route it authenticates shipped in v3.2.1. **Required in production** — every `/api/cron/*` route returns `503` until this is set, and `401` for any request whose bearer token doesn't match it. Optional at the env-schema level only so the app still boots in a fresh environment before this is set. Only `/api/cron/ahaana-weekly-report` (weekly) still runs on Vercel's own cron feature (`vercel.json`, sends `Authorization: Bearer $CRON_SECRET` automatically) — Hobby caps that feature to once a day. The three more-frequent routes (`reminders` every 4h, `reminders-hourly` and `ahaana-reminders` every 15min) run on **[cron-job.org](https://cron-job.org)** (free account; create one job per route, method GET, custom header `Authorization: Bearer <the same CRON_SECRET value>`, schedule matching each route's cadence). v3.6.9 tried GitHub Actions' own `schedule:` trigger for this first — v3.7.3 measured its real run history and found it silently delivers every 2-7 hours instead of the configured 15min/4h, which is what actually caused a household reminder to never fire (see `docs/00-current-state.md`'s v3.7.3 section). The `.github/workflows/cron-*.yml` files still exist for **manual testing only** (`workflow_dispatch`, no `schedule:` trigger) — still need the same `CRON_SECRET` **GitHub repository secret** for that to work, but nothing here relies on GitHub's own scheduler anymore. |
...
| `TELEGRAM_WEBHOOK_SECRET`                | Generate yourself: `openssl rand -hex 32`                                   | Added in v3.7.0. Authenticates `/api/telegram/webhook` (a household group message containing "remind" auto-creates a real calendar event) — Telegram echoes this value back verbatim as `X-Telegram-Bot-Api-Secret-Token` on every Update POST, but only once the webhook is actually registered with a matching `secret_token`, a one-time manual step after this is set in Vercel and deployed: `curl -X POST "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/setWebhook" -H "Content-Type: application/json" -d '{"url": "https://expdash.vercel.app/api/telegram/webhook", "secret_token": "<this value>"}'`. Optional at the env-schema level, same reasoning as `CRON_SECRET` — the route returns `503` until this is set, `401` if a request's header doesn't match it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
