...
`HDFC_INFINIA_STATEMENT_PASSWORD`, `AXIS_STATEMENT_PASSWORD`, and
`ICICI_STATEMENT_PASSWORD` is required — the app fails fast (loudly, on
startup) if any are missing or malformed, rather than running with a gap.
