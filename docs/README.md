...
- Feature set as of v1.7.3: ledger core (accounts, transactions, budgets,
  recurring, net worth, attachments), credit-card statement imports (HDFC
  Infinia, Axis Horizon) with a shared Merchant Dictionary, an Intel tab
  with charts and a button-triggered AI insight, and a Calendar tab
  (school calendar + trips) that's the one public, gate-free route.
