...
- Feature set as of v1.9.0: ledger core (accounts, transactions, budgets,
  recurring, net worth, attachments), credit-card statement imports (HDFC
  Infinia, Axis Horizon, ICICI Amazon Pay / RuPay) with a shared Merchant
  Dictionary, an Intel tab with charts and a button-triggered AI insight,
  and a Calendar tab (school calendar + trips) that's the one public,
  gate-free route.
