...
// same convention as the other cron route tests.
vi.mock("server-only", () => ({}));
...
const runAhaanaWeeklyReportCronMock = vi.fn();
vi.mock("@/services/ReminderService", () => ({
  runAhaanaWeeklyReportCron: (...args: unknown[]) =>
    runAhaanaWeeklyReportCronMock(...args),
}));
...
function requestWithAuth(header?: string): Request {
  ...
  return new Request("http://localhost/api/cron/ahaana-weekly-report", {
    headers,
  });
}

describe("GET /api/cron/ahaana-weekly-report", () => {
  beforeEach(() => {
    mockServerEnv.CRON_SECRET = "test-cron-secret";
    mockServerEnv.APP_SESSION_SECRET = "a".repeat(32);
    runAhaanaWeeklyReportCronMock.mockReset();
    runAhaanaWeeklyReportCronMock.mockResolvedValue({
      candidates: 1,
      sent: 1,
      skipped: 0,
      failed: 0,
  ...
  it("rejects a request with no Authorization header", async () => {
    ...
    expect(runAhaanaWeeklyReportCronMock).not.toHaveBeenCalled();
  });
  ...
  it("runs the weekly report and returns the result for a valid bearer token", async () => {
    const res = await GET(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    expect(runAhaanaWeeklyReportCronMock).toHaveBeenCalledTimes(1);
  });
});
