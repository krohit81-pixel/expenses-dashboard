 ...
 * "1,51,388.00"), a real Axis statement prints at least one figure with
 * NO thousands separator at all -- the eDGE Miles points balance
 * ("26276", not "26,276") on a Horizon statement. The original
 * HDFC-derived pattern (`\d{1,3}(?:,\d{2,3})*`) silently truncated an
 * ungrouped multi-digit number at the first 3 digits (matching just
 * "262" out of "26276"), since `(?:,\d{2,3})*` allows zero repetitions
 * and a bare digit run has no comma to require more. The alternation
 * below tries the comma-grouped shape first (still required to have at
 * least one comma group, so it won't mis-split an ungrouped number
 * early) and falls back to a plain digit run.
 */
